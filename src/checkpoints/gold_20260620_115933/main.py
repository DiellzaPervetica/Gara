...
def construct_mandatory_vrp(instance: Instance, seconds: int) -> Solution | None:
    ...
    for street, group in groups:
        ...
        for index in indices:
            routing.SetAllowedVehiclesForIndex(eligible, index)
...
def local_search(solution: Solution, instance: Instance, paths: ShortestPaths, rng: random.Random) -> None:
    ...
    for _ in range(12):
        ...
        if not moved and not relocated and not oriented and before == after:
            break
...
def solve(
    ...
    for seed in range(attempts):
        ...
        constructor = construct_mandatory_large if sum(street.category == "M" for street in instance.streets) > 100 else construct_mandatory
        candidate = constructor(instance, paths, rng)
        if candidate is None:
            if constructor is construct_mandatory:
                # Keep the diversified stream independent from the regret stream;
                # this makes a failed regret attempt a genuine fresh restart.
                candidate = construct_mandatory_randomized(instance, paths, random.Random(9 + seed))
        if candidate is None:
            continue
